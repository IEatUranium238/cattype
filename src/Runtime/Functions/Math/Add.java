...
public class Add {
  public static Object add(Map<String, Object> data, int lineNum) throws Exception {
    @SuppressWarnings("unchecked")
    ArrayList<Object> content = (ArrayList<Object>) data.get("content");
    if (content.size() != 2) {
      throw new Exception("ADD accepts only 2 parameters on line: " + lineNum);
    }
    ...
    if (paramL.get("type").equals("FUN")) {
      try {
        ...
        throw new Exception(
            "ADD accepts only INT values, got invalid value type or format from function on line: " + lineNum);
      }
    } else {
      if (String.valueOf(paramL.get("content")).matches(".*[a-zA-Z'\"].*") ) {
        throw new Exception(
            "ADD accepts only INT values, got invalid value type or format from input on line: " + lineNum);
      }
